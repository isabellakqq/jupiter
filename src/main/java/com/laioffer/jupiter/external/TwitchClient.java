...
public class TwitchClient {
    private static final String TOKEN = "Bearer pr1pr1knvii7ut2ctuavwvjnfclz9g";
    private static final String CLIENT_ID = "00gib1p2na2hha8huzsisu4wcbfmvz";
    ...
    private String searchTwitch(String url) throws TwitchException {
        ...
        ResponseHandler<String> responseHandler = response -> {
            ...
            // 将response data转换为 Java Jsonobject 格式
    ...
    // Returns the top x streams based on game ID. 每一个 gameId 会有很多个 streams/videos/clips（因为不同的主播可能会做同一个游戏的视频），所以返回的是一个 List
    private List<Item> searchStreams(String gameId, int limit) throws TwitchException {
        ...
        for (Item item : streams) {
            ...
            // 只有 stream 的 json 里没有返回 url，所以需要手动设置
    ...
    public List<Item> searchByType(String gameId, ItemType type, int limit) throws TwitchException {
        ...
        // Update gameId for all items. GameId is used by recommendation function. 只有 video 的 JSON data 没有返回 gameId，所以需要手动 set gameId.
        for (Item item : items) {
